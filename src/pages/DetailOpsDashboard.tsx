import { IconLayoutDashboard, IconArrowRightCircle, IconBrandGithub, IconBrandVercel, IconCircleCheck, IconCircle, IconClock, IconTag, IconStar } from '@tabler/icons-react';
import FadeIn from '../components/FadeIn';

const PROJECT = {
  id: '12',
  title: 'OPS DASHBOARD',
  subtitle: '마케팅 자동화 내부 대시보드',
  description: 'Phase 0 유입·전환 검증을 위한 내부 직원용 운영 대시보드. 6주간의 파일럿 진행 상황, 직원별 업무, 핵심 지표, Go/No-Go 판단을 한 화면에서 확인할 수 있습니다.',
  since: '2026.04',
  status: 'active' as const,
  siteUrl: 'https://phase0-dashboard-app.vercel.app',
  githubUrl: 'https://github.com/ork7483-byte/sns-agency-dashboard',
  vercelUrl: 'https://vercel.com/vision-ai/phase0-dashboard-app',
};
...
const TECH_STACK = [
  ...
  { name: 'Recharts', color: 'bg-green-100 text-green-700' },
];

const TIMELINE = [
  { date: '2026.04.05', content: '사업계획서 다운로드 버튼 추가', tag: 'feature' },
  { date: '2026.04.04', content: 'TypeScript 에러 수정 및 Vercel 배포', tag: 'deploy' },
  { date: '2026.04.04', content: 'GitHub 레포 생성 (sns-agency-dashboard)', tag: 'infra' },
  { date: '2026.04.03', content: '상세 드릴다운 페이지 10개 구현 (직원/지표/업무/AB테스트/템플릿/레퍼런스/콘텐츠/리포트/알림)', tag: 'feature' },
  { date: '2026.04.03', content: 'ForestView 대시보드 메인 화면 제작', tag: 'feature' },
  { date: '2026.04.02', content: 'Vite + React 19 + shadcn/ui 프로젝트 초기 셋업', tag: 'infra' },
];
...
const TODOS = [
  { done: true, text: 'Phase 0 대시보드 메인 화면' },
  { done: true, text: '직원별 상세 드릴다운' },
  { done: true, text: '유입/전환 지표 상세' },
  { done: true, text: 'Go/No-Go 판단 카드' },
  { done: true, text: '사업계획서 다운로드' },
  { done: true, text: 'Vercel 배포' },
  { done: false, text: '실제 API 연동 (Supabase)' },
  { done: false, text: '실시간 데이터 업데이트' },
  { done: false, text: '알림/Slack 연동' },
];

const TAGS = ['내부도구', '대시보드', 'Phase 0', '마케팅 자동화'];
...
export default function DetailOpsDashboard() {
  ...
  return (
    <div className="min-h-screen bg-white font-sans pt-20 md:pt-24 pb-24">
      ...
      <section className="w-full pt-6 pb-8 px-6 relative overflow-hidden">
        ...
        <div className="absolute top-1/4 left-1/4 w-64 h-64 bg-indigo-600 rounded-full mix-blend-screen filter blur-[80px] opacity-60 animate-pulse" />
        <div className="absolute bottom-1/4 right-1/4 w-80 h-80 bg-violet-500 rounded-full mix-blend-screen filter blur-[100px] opacity-50" />
            ...
            <div className="flex flex-col md:flex-row items-center justify-center gap-4 md:gap-6 mb-4">
              <div className="w-16 h-16 md:w-20 md:h-20 rounded-2xl rotate-12 bg-gradient-to-br from-white/40 to-white/5 border border-white/30 flex items-center justify-center text-white shadow-[0_10px_20px_rgba(0,0,0,0.2)]">
                <div className="-rotate-12">
                  <IconLayoutDashboard size={40} stroke={1.5} />
                </div>
              </div>
      ...
      <section className="max-w-7xl mx-auto px-6 md:px-12 py-16 md:py-24">
        <div className="flex flex-col md:flex-row gap-12 md:gap-16">
          ...
          <div className="w-full md:w-[260px] shrink-0">
            <FadeIn>
              <div className="text-[#3b4859] font-bold mb-4 text-sm uppercase tracking-widest">
                Service.{PROJECT.id}
              </div>
              ...
              <div className="flex flex-col gap-3 mb-8">
                {LINKS.map((link) => (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex items-center gap-2 bg-[#4a5568] hover:bg-[#2d3748] text-white px-5 py-3 text-sm font-bold transition-colors w-full justify-center"
                  >
                    <link.icon size={18} />
                    {link.label}
                  </a>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {TECH_STACK.map((tech) => (
                    <span key={tech.name} className={`px-3 py-1 text-xs font-semibold rounded-full ${tech.color}`}>
                      {tech.name}
                    </span>
                  ))}
                </div>
            ...
            <FadeIn delay={0.1}>
              <div className="bg-gray-50 rounded-2xl p-6 md:p-8">
                <div className="flex items-center gap-2 mb-6">
                  ...
                  <h3 className="text-lg font-bold text-gray-900">업데이트 타임라인</h3>
                </div>
                  ...
                  {TIMELINE.map((item, i) => (
                    <div key={i} className="relative">
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3">
                        ...
                        <span className={`text-[11px] font-bold px-2 py-0.5 rounded-full ${TAG_COLORS[item.tag] || 'bg-gray-100 text-gray-600'}`}>
                          {item.tag}
                        </span>
                      </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div className="bg-white rounded-xl p-4 border border-gray-100">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-sm font-bold text-gray-700">사이트</span>
                    </div>
                  ...
                  <div className="bg-white rounded-xl p-4 border border-gray-100">
                    <div className="flex items-center gap-2 mb-2">
                      <div className="w-2.5 h-2.5 rounded-full bg-green-500 animate-pulse" />
                      <span className="text-sm font-bold text-gray-700">Vercel</span>
                    </div>
                  ...
                  <div className="bg-white rounded-xl p-4 border border-gray-100">
                    ...
                    <p className="text-xs text-gray-500">master · 최신</p>
                  </div>
                </div>
              </div>
            </FadeIn>
                  ...
                  {TODOS.map((todo, i) => (
                    <div key={i} className="flex items-start gap-3">
                      {todo.done ? (
                        ...
                        <IconCircle size={20} className="text-gray-300 shrink-0 mt-0.5" />
                      )}
              ...
              <div className="bg-gray-50 rounded-2xl p-6 md:p-8">
                <div className="flex items-center justify-between mb-6">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-lg font-bold text-gray-900">QA 리뷰</h3>
                  </div>
                ...
                <div className="flex items-center gap-4 mb-6 p-4 bg-white rounded-xl border border-gray-100">
                  ...
                  <div>
                    <div className="text-sm font-bold text-gray-700">종합 점수</div>
                    <div className="text-xs text-gray-400">10점 만점</div>
                  </div>
                  <div className="ml-auto flex gap-0.5">
                    {[...Array(5)].map((_, i) => (
                      <IconStar
                        key={i}
                        size={18}
                        className={i < Math.round(QA_REVIEW.score / 2) ? 'text-yellow-400 fill-yellow-400' : 'text-gray-200'}
                      />
                    ))}
                  </div>
                </div>
