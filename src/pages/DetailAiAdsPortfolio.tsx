import { IconPhotoAi, IconArrowRightCircle, IconBrandGithub, IconBrandVercel, IconCircleCheck, IconCircle, IconClock, IconTag, IconStar } from '@tabler/icons-react';
...
const PROJECT = {
  ...
  title: 'AI AD AGENCY',
  subtitle: 'AI 광고 에이전시',
  description: '제품 이미지 하나로 AI가 고품질 광고 콘텐츠를 제작. 룩북, 배너, SNS 콘텐츠 등 다양한 광고 소재를 자동으로 생성하여 마케팅 효율을 극대화합니다.',
  since: '2024.01',
  status: 'active' as const,
  siteUrl: 'https://crawled.vercel.app',
  githubUrl: 'https://github.com/ork7483-byte/visionai-ads',
  vercelUrl: 'https://vercel.com/vision-ai/crawled',
};
...
const TECH_STACK = [
  ...
  { name: 'AI Image Generation API', color: 'bg-green-100 text-green-700' },
];
...
const TODOS = [
  { done: true, text: '광고 콘텐츠 생성 엔진' },
  { done: true, text: '룩북 자동 생성' },
  { done: true, text: '배너 디자인 자동화' },
  { done: true, text: 'SNS 콘텐츠 템플릿' },
  { done: true, text: 'Vercel 배포' },
  { done: false, text: 'API 안정화' },
  { done: false, text: '다국어 지원' },
];

const TAGS = ['AI 서비스', '광고', '콘텐츠 제작'];
...
export default function DetailAiAdsPortfolio() {
  ...
  return (
    <div className="min-h-screen bg-white font-sans pt-20 md:pt-24 pb-24">
      ...
      <section className="w-full pt-6 pb-8 px-6 relative overflow-hidden">
        <div className="absolute inset-0 z-0 bg-gradient-to-br from-orange-900 to-red-800" />
        <div className="absolute top-1/4 left-1/4 w-64 h-64 bg-orange-500 rounded-full mix-blend-screen filter blur-[80px] opacity-60 animate-pulse" />
        <div className="absolute bottom-1/4 right-1/4 w-80 h-80 bg-yellow-500 rounded-full mix-blend-screen filter blur-[100px] opacity-50" />
          ...
          <div className="w-full md:w-[260px] shrink-0">
            <FadeIn>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {TECH_STACK.map((tech) => (
                    <span key={tech.name} className={`px-3 py-1 text-xs font-semibold rounded-full ${tech.color}`}>{tech.name}</span>
                  ))}
                </div>
              </div>
            </FadeIn>
            ...
            <FadeIn delay={0.2}>
              <div className="bg-gray-50 rounded-2xl p-6 md:p-8">
                ...
                <div className="w-full bg-gray-200 rounded-full h-2 mb-5">
                  <div className="bg-green-500 h-2 rounded-full transition-all" style={{ width: `${(TODOS.filter(t => t.done).length / TODOS.length) * 100}%` }} />
                </div>
              ...
              <div className="bg-gray-50 rounded-2xl p-6 md:p-8">
                ...
                <div className="flex items-center gap-4 mb-6 p-4 bg-white rounded-xl border border-gray-100">
                  ...
                  <div className="ml-auto flex gap-0.5">
                    {[...Array(5)].map((_, i) => (
                      <IconStar key={i} size={18} className={i < Math.round(QA_REVIEW.score / 2) ? 'text-yellow-400 fill-yellow-400' : 'text-gray-200'} />
                    ))}
                  </div>
                </div>
                <div className="space-y-3">
                  {QA_REVIEW.items.map((item) => (
                    <div key={item.area} className="flex items-center gap-3">
                      ...
                      <div className="flex-1 bg-gray-200 rounded-full h-2">
                        <div className="bg-yellow-400 h-2 rounded-full" style={{ width: `${item.score * 10}%` }} />
                      </div>
